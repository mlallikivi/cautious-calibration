...
# IMPORTANT: Mari-Liis changed the method because it run into an infinite loop if k = n - 1.
# I don't know exactly, but my intuition is that it is not possible to find the credible interval
# with the requirements in this case. So I made a change that after running for 1000 cycles 
# difference is not below tolerance or if the logpdf of one of the probabilities is infinite,
# the cycle is discontinued and the resulting interval is returned. I don't know what
# the properties are for this interval, does it still cover the required confidence level?
# But as I am using it just as a comparison and it has some theoretical mistakes anyways,
# (prior assumption, grouping etc..) then I don't think it is a big deal.
            ...
            while(abs(beta.logpdf(p_min_middle, k + 1, n - k + 1) -
                ...
                if(k * np.log(p_min_middle) + (n - k) * np.log(1 - p_min_middle) >
                   k * np.log(p_max_middle) + (n - k) * np.log(1 - p_max_middle)):
                    ...
                    p_max_lower = p_max_middle
                p_max_middle = (p_max_lower + p_max_upper) / 2
                if abs(beta.logpdf(p_max_middle, k + 1, n - k + 1)) == np.inf or inner_loop_counter == 1000:
                    break
...
## MARI-LIIS modified:  I added "and ..." part to the last while loop, to also check that
## there would be more than 1 bin left, otherwise merging can't take place
def train_rcir(training_class, training_scores,
    ...
    #return bin_summary    
    #print("hello1")
    # I changed the tolerance from 1e-6 to 1e-3 for some of my cases to not go
    # to infinite loops
    credible_intervals = [credible_interval(np.round(p * n), n, tolerance=1e-6) for (p, n) in
    ...
    merge_counter = 0
    while(max(rcir_model['width of intervals']) > d and len(rcir_model['width of intervals']) > 2):
        # Merge one more bin.
        #print(merge_counter)
        merge_counter += 1
